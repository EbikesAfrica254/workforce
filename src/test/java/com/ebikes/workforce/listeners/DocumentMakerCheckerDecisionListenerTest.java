...
@DisplayName("DocumentMakerCheckerDecisionListener")
@ExtendWith({MockitoExtension.class, WithExecutionContext.class})
class DocumentMakerCheckerDecisionListenerTest {

  @Mock private DocumentApprovalHandler documentApprovalHandler;
  @Mock private InboxService inboxService;
  @Spy private ObjectMapper objectMapper;

  @InjectMocks private DocumentMakerCheckerDecisionListener handler;
  ...
  class Handle {
    ...
    void shouldSkipDispatchOnDuplicateEvent() {
      MakerCheckerDecision decision =
          MakerCheckerFixtures.approved(DocumentFixtures.DOCUMENT_ID, "DOCUMENT", "REPLACE");
      when(inboxService.receive(any(), any(), any())).thenReturn(false);

      handler.handle(serialize(decision));

      verify(documentApprovalHandler, never()).handleDecision(any());
    ...
    void shouldDispatchAndMarkProcessedOnNewEvent() {
      ...
      verify(documentApprovalHandler).handleDecision(decision);
  ...
  class Matches {
    ...
    @Test
    @DisplayName("should return false for a workforce maker-checker routing key")
    void shouldReturnFalseForWorkforceRoutingKey() {
      assertThat(handler.matches(ExternalContracts.MAKER_CHECKER_WORKFORCE_PREFIX + "agent"))
          .isFalse();
    }

    @Test
    @DisplayName("should return true for a document maker-checker routing key")
    void shouldReturnTrueForDocumentRoutingKey() {
